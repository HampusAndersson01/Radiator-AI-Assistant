...
def get_weather(lat=59.5, lon=16):
    """Return current outside temp, +3h forecast, and +10h forecast using SMHI API.

    Returns (current, forecast_3h, forecast_10h)
    
    SMHI provides hourly forecasts up to 15 days ahead.
    """
    ...
    try:
        ...
        forecast_10h_temp = None
        
        now = datetime.now(timezone.utc)
        target_3h = now + timedelta(hours=3)
        target_10h = now + timedelta(hours=10)
        ...
        for i, forecast in enumerate(time_series):
            ...
            # Find closest forecast to +10h
            if forecast_10h_temp is None and valid_time >= target_10h:
                forecast_10h_temp = temp
                print(f"SMHI: +10h forecast = {temp}°C at {valid_time}")
                break
        ...
        if forecast_10h_temp is None and len(time_series) > 10:
            for param in time_series[min(10, len(time_series)-1)].get("parameters", []):
                if param.get("name") == "t":
                    values = param.get("values", [])
                    if values and len(values) > 0:
                        forecast_10h_temp = values[0]
                    break
        
        # Final fallback
        if forecast_3h_temp is None:
            forecast_3h_temp = current_temp
        if forecast_10h_temp is None:
            forecast_10h_temp = forecast_3h_temp
        
        print(f"SMHI: Returning current={current_temp}, 3h={forecast_3h_temp}, 10h={forecast_10h_temp}")
        return current_temp, forecast_3h_temp, forecast_10h_temp
